...
pub fn main(init: std.process.Init) !void {
    ...
    var in_file = try std.Io.Dir.cwd().openFile(io, scripts_data_path, .{});
    ...
    for (0..0x110000) |i| {
        ...
        const stage3_idx = blk: {
            ...
            try stage3.append(arena, script);
    ...
    try c.writeInt(u16, @intCast(stage1.items.len));
    try c.writeInt(u16, @intCast(stage2.items.len));
    try c.writeInt(u16, @intCast(stage3.items.len));
    for (stage1.items) |i| try c.writeInt(u16, i);
    for (stage2.items) |i| try c.writeInt(u8, i);
    for (stage3.items) |i| try c.writeInt(u8, i);
